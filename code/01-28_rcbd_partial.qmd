...
# e) Statistical model  (Feb 4, 2025)
...
```{r model}
# Changing to sum-to-zero contrast (this is the default in SAS). Applies a restriciton to the model. 
options(contrasts = c("contr.sum", "contr.poly"))

# Model fitting
rcbd_mod <- lm(yield_kgha ~ rep + nrate_kgha * krate_kgha,  # adding reps/blocks is the difference between cbd and rcbd
               data = rcbd_dfw)

# Summary
summary(rcbd_mod) # this is mainly for regression
```
...
```{r ANOVA}
Anova(rcbd_mod, type=3) # type 3 handles balanced and unbalanced data
```
...
# emmeans: expected marginal means

rcbd_means_all <- emmeans(rcbd_mod, ~ nrate_kgha * krate_kgha ) # compare everything to everything
rcbd_means_all

rcbd_means_all <- emmeans(rcbd_mod, ~ nrate_kgha | krate_kgha ) # N rate inside K rate
rcbd_means_all

rcbd_means_all <- emmeans(rcbd_mod, ~  krate_kgha | nrate_kgha ) # K rate inside N rate (comparisons of K are restricted to each N rate)
rcbd_means_all


# another way 
rcbd_mod %>% emmeans(~ nrate_kgha : krate_kgha ) %>% 
  cld (reversed = T, adjust = "none", Letters= letters)
```
...
rcbd_cld_all <- cld(rcbd_means_all, 
                   reversed=T, # from smallest to largest 
                   adjust="none", # the more comparisons the more false positivies we'll find (Tukey is common)
               Letters=letters) # we can add an alpha of 0.1 (alpha = 0.1) instead of 0.5
...
rcbd_cld_selected <- rcbd_cld_all %>% 
  as.data.frame() %>% 
  mutate(letter = trimws(.group)) %>%  # trimws: trim white space
  mutate(trtname = paste0(nrate_kgha, "+", krate_kgha)) # create a column of treatments combinations
...
```{r rcbd final plot}
# we are gonna use the raw data and means data
...
ggplot() + 
  ...
  geom_jitter(data = rcbd_dfw, 
               aes(x = trtname, y = yield_kgha, color = trtname), alpha = 0.6) + 
  
  # letters 
  geom_label(data = rcbd_cld_selected,        # we could use geom_label but gives a box
            aes(x = trtname, y = emmean, label = letter), # emmean colum has the mean. Use letter columns instead of .group
            size = 3, label.padding = unit(.1, "lines")) %>%  
  labs(x = "N rate + K rate (Kg/rate)",
       y = "Yield (Kg/ha)") + 
...
Comparing K rate inside N rate
```{r}
  # raw data 
ggplot() + 
  geom_boxplot(data = rcbd_dfw, 
  ...
  geom_jitter(data = rcbd_dfw, 
               aes(x = krate_kgha, y = yield_kgha, color = krate_kgha), alpha = 0.6) + 
  
  # letters 
  geom_label(data = rcbd_cld_selected,        # we could use geom_label but gives a box
            aes(x = krate_kgha, y = emmean, label = letter), # emmean colum has the mean. Use letter columns instead of .group
            size = 3, label.padding = unit(.1, "lines")) %>%  
  labs(x = "K rate (Kg/rate)",
       y = "Yield (Kg/ha)") + 
  scale_fill_viridis_d() + 
  scale_color_viridis_d() + 
