...
# Model fitting
rcbd_mix_mod <- lmer(yield_kgha ~ nrate_kgha * krate_kgha + # fixed effect before (nrate_kgha * krate_kgha + block)
                       (1|rep), #random effects not connected or independent ((1|block) + (1|location)) 
                     data = rcbd_dfw) 

# the larger hierarchy first ((1|location/block)), which sames as ((1|location) + 1|location:block). 
# For year, it would be (1|year/rep). For year, location, and rep, it would be (1|year:locatio/rep) interaction, (1|year/locatio/rep) interaction, or (1|location/rep) or create a new column (1|location_year/rep)

# Summary
summary(rcbd_mix_mod)

## summary explanation
# REML was able to converge, and was able to estimate
# residuals are the eij
#random effects. A lot more error in the residual. Having a block allows me to take measurements by block or deal with constraints in the field if we can not finish everything the same day. 
# the fixed effects we do not pay much attention. Better to look at Anova()
```
...
# f) ANOVA table  
```{r ANOVA}
Anova(rcbd_mix_mod, type=3) # anova tables only show the fixed effects 
```
...
# the SE's changes when we include the random effects. The means are the same. 
```
...
Compare between fixed-effect and random-effect models on the points below. Describe similarities or discrepancies clearly stating which model create lower/higher/different answers.

1) ANOVA table p-values

2) emmeans standard error 

3) Letter separation



- Although we have different values for probability, the interaction of nrate and krate are significant for both fixed and mixed models. The degrees of freedom are the same, except that the fixed model gives us the ones for the block. 
- The model means are the same for fixed and mixed models, however, the standard errors are higher for the mixed model. 
- The letter separation looks the same for both models, the overall interpretation thus does not change.  

Question for next class. 
 - should not the Standard Erros be lower for the mixed model since we are accounting for the block in the model. 
 - what does it mean the (1|) in the model? 
 - 
